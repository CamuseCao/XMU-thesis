...
- 该模板应该能够在`Mac`和`Windows`系统下运行。`windows`用户无需更改配置，但是`Mac`用户请打开`Settings/Seetting.tex`文件下，把89-91这三行代码注释掉，然后把93-95行的代码去掉注释，即可运行。~~最近事情多，没用虚拟机去测试Linux。有同学测试了可以和我反应一下结果啊。~~
